...
und anschließend auf den Button _+ Create_. Nun gibt man einen Namen, z.B. `flug` ein
...
Für das vorliegende Beispiel müssen deshalb für das Environment `flug` folgende Werte
gesetzt werden:
...
Mit _JavaScript_ kann man Requests zu einem Server schicken, um z.B. einen Token anzufordern
oder um Daten zu Metriken bei _Spring Boot Actuator_ zu generieren. Wenn bei einem solchen
Server selbst-signierte Zertifikate verwendet werden, kann man allerdings **nicht** die
Funktion `fetch` aus ECMAScript 2015 verwenden, sondern man muss `bru.sendRequest` verwenden
und vom _Safe Mode_ in den _Developer Mode_ wechseln.
Voraussetzung dafür ist, dass auf dem eigenen Entwicklungsrechner _Node_ installiert ist.
Zum Wechseln in den _Developer Mode_ klickt man rechts oben auf _Safe Mode_ und
anschließend auf den Radiobutton _Developer Mode_.
