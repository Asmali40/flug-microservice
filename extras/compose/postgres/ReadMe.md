...
neue DB `Flug` mit dem DB-User `kunde`anzulegen. Mit dem 2. Skript wird das
Schema `kunde` mit dem DB-User `kunde` als _Owner_ angelegt:

```powershell
   docker compose exec db bash
      psql --dbname=postgres --username=postgres --file=/sql/create-db-flug.sql
      psql --dbname=kunde --username=kunde --file=/sql/create-schema-flug.sql
      exit
    docker compose down
```
